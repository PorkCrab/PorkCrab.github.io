...
public class EffectPoolUnit : MonoBehaviour
{
    ...
    //Active 껐을때의 시간 꺼졌을때부터 1초 지난 거 체크위해 사용
    ...
    DESTROY_TYPE m_destroy = DESTROY_TYPE.Inactive;
    ...
    public bool IsReady()
    {
        if (!gameObject.activeSelf) //꺼져있으면 풀에 들어가있음을의미
        {
            ...
            //현재시간 - 엑티브 껏을때 시간 //timeSpan으로 값이나옴
            if (timeSpan.TotalSeconds > m_delay)
            {   //시간을 전체 시 / 분 / 초 로 나눠서 받을수있다. 
                //timeSpan.TotalSeconds 초로만 반환을 했을때 1초보다 크면

                //엑티브가 꺼진지 1초이상 지나면 이펙트 
                //여러개터트려도 문제가 발생안되서 1초 조건 걸음
                return true;
            }
        }
        return false;
